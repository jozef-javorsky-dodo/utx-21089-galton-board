...
func (b *Board) Simulate(ctx context.Context) ([]uint64, error) {
	...
	tasks := make([]any, b.cfg.NumWorkers)
	...
	observable := rxgo.Just(tasks...)(rxgo.WithContext(ctx))
	simulated := observable.Map(b.mapTask, rxgo.WithPool(b.cfg.NumWorkers))
	reduced := simulated.Reduce(b.reduceResults)

	var finalDist []uint64
	for item := range reduced.Observe(rxgo.WithContext(ctx)) {
		if item.Error() {
			return nil, item.E
		}
		finalDist = item.V.([]uint64)
	}

	if finalDist == nil {
		return nil, errors.New("no distribution generated")
	}
...
func (b *Board) mapTask(ctx context.Context, item any) (any, error) {
	task := item.(SimulationTask)

	localDist := make([]uint64, b.cfg.Width)
	rng := rand.New(rand.NewPCG(task.Seed1, task.Seed2))

	for i := uint64(0); i < task.Balls; i++ {
		if i&1023 == 0 {
			select {
			case <-ctx.Done():
				return nil, ctx.Err()
...
func (b *Board) reduceResults(_ context.Context, acc any, elem any) (any, error) {
	if acc == nil {
		return elem, nil
	}

	aggregate := acc.([]uint64)
	localDist := elem.([]uint64)
	for i, val := range localDist {
		aggregate[i] += val
	}
